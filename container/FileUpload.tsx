...
const FileUpload = () => {
  ...
  const getCurrentFolderId = () => {
    switch (activeTab) {
      case "hajj":
        return hajjFolderId;
      case "umrah":
        return umrahFolderId;
      case "madina":
        return madinaFolderId;
      default:
        return null;
    }
  };
  ...
  const handleUpload = async () => {
    if (
      ...
      !contentImageFile &&
      !name.trim() &&
      description.every((d) => !d.trim()) &&
      paragraphs.every(
        (p) => !p.title.trim() && p.description.every((d) => !d.trim())
      )
    ) {
    ...
    try {
      ...
      const hasContent =
        ...
        paragraphs.some(
          (p) => p.title.trim() || p.description.some((d) => d.trim())
        );
      ...
      if (hasContent || contentImageUrl) {
        try {
          ...
          if (filesToUpload.length > 0) {
            const contentMetaRef = ref(
              storage,
              `${activeTab}/${currentFolderId}/content_metadata.json`
            );
      ...
      <div className="space-y-6">
        ...
        <div className="flex justify-end">
          ...
          >
            {viewMode === "form" ? (
              <>
                <FaList className="mr-1" /> View Content List
              </>
            ) : (
              <>
        ...
        {/* Form or List View */}
        {viewMode === "form" ? (
          /* Form view - existing content form */
          <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200">
                ...
                {contentImagePreview ? (
                  ...
                  >
                    ...
                    <p className="text-gray-500 text-sm">
                      Upload a featured image for your content
                    </p>
                    <p className="text-xs text-gray-400 mt-1">
                      This image will be stored with your content
                    </p>
                  </div>
                )}
                <input
              ...
              {paragraphs.map((para, paraIndex) => (
                ...
                >
                  ...
                  <input
                    ...
                    value={para.title}
                    onChange={(e) =>
                      handleParagraphTitleChange(paraIndex, e.target.value)
                    }
                  ...
                  {para.description.map((desc, descIndex) => (
                    <div
                      key={`para-${paraIndex}-desc-${descIndex}`}
                    ...
                    >
                      ...
                      {para.description.length > 1 && (
                        <button
                          type="button"
                          onClick={() =>
                            removeParagraphDescription(paraIndex, descIndex)
                          }
                          className="absolute top-2 right-2 text-red-500 hover:text-red-700"
                        >
                          <MdDelete />
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Choose Files{" "}
                  {otherFiles.length > 0 && `(${otherFiles.length} selected)`}
                </label>
                <input
                  type="file"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Upload Audio{" "}
                  {audioFiles.length > 0 && `(${audioFiles.length} selected)`}
                </label>
                <input
                  type="file"
                  accept="audio/*"
            ...
            <button
              ...
              className="w-full bg-blue-600 text-white py-2 px-4 rounded-md hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors mt-4"
            >
              {editMode ? "Update Content" : "Upload Content"}
            </button>
          </div>
        ) : (
          <ContentList activeTab={activeTab} />
        )}
